...
const employeeCreate = async (req, res) => {
  var {employee_id, f_name, l_name, email, phone, role, password, sup_id } = req.body;
  sup_id= Number(sup_id)===0?0:sup_id;
  try {
    const sql = `insert into employee_master(employee_id,f_name,l_name,phone,email,role,password,sup_id) values ("${employee_id}","${f_name}","${l_name}","${phone}","${email}","${role}","${password}","${sup_id}")`;
